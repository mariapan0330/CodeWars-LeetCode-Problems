...
If we are limited to digits 0..9, the biggest number we can encode 
is 10!-1 (= 3628799). So we extend 0..9 with letters A..Z. 
With these 36 digits we can now encode numbers up to 36!-1 (= 3.72 * 1041)

Task
We will need two functions. The first one will receive a decimal 
number and return a string with the factorial representation.
...
def dec_2_fact_string(nb):
    ...
    count = 1
    while div > 0:
        equ += f"{div % count}*math.factorial({count-1})+"
        div //= count
        print('x')
    print(equ)
