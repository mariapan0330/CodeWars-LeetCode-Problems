...
def encode_rail_fence_cipher(message, n):
    if message == '':
        return ''
    ...
    for x in range(0, len(message), interval):
        res+=message[x]
...
def decode_rail_fence_cipher(message, n):
    pass



ex1 = 'WEAREDISCOVEREDFLEEATONCE!'
# 2    ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^
# 3    ^   ^   ^   ^   ^   ^   ^
# 4    ^     ^     ^     ^     ^

print(encode_rail_fence_cipher(ex1, 2))
print(encode_rail_fence_cipher(ex1, 3))
print(encode_rail_fence_cipher(ex1, 4))